...
const Portfolio = () => {
	...
	const [displayCount, setDisplayCount] = useState(3); // Initial number of projects to display
	...
	const filterItems = (categoryItem) => {
		...
		setDisplayCount(3); // Reset to initial display count when filtering
	};

	const loadMoreProjects = () => {
		setDisplayCount((prevCount) => prevCount + 3); // Load 4 more projects
	};
	...
	return (
		<section className="portfolio container section" id="portfolio">
			...
			<div className="portfolio__filters">
				<span className={activeFilter === 0 ? 'portfolio__item portfolio__item-active' : 'portfolio__item'} onClick={() => { setItems(Menu); setActiveFilter(0); setDisplayCount(4); }}>
					All
				</span>
			...
			<div className="portfolio__container grid">
				{items.slice(0, displayCount).map((elem) => {
			...
			{displayCount < items.length && (
				...
				<button className="link-load" onClick={loadMoreProjects}>Load More</button>
				</div>
			)}
		</section>
	);
